...
const Message: React.FC<MessageProps> = ({ message, duration = 3000, setsuccess, setMessage, success, setError }) => {
    ...
    return (
        <div className={`${success? 'success': 'error'} message ${!isVisible ? 'fade-out' : ''}`}>
            {message}
        </div>
    );
};
