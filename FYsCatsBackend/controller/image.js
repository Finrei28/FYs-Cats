...
const getImage = async (req, res) => {
  ...
  if (!image) {
    return res
      ...
      .json({ msg: `Image could not be found` });
  }
  res.status(StatusCodes.OK).json({ image });
};
...
const deleteImage = async (req, res) => {
  ...
  try {
    ...
    if (result.deletedCount === 0) {
      return res
        .status(StatusCodes.NOT_FOUND)
        .json({ msg: "No images were found to delete" });
    }

    res
      .status(StatusCodes.OK)
      .json({ msg: `${result.deletedCount} images deleted successfully` });
  } catch (error) {
    console.error("Error deleting images:", error);
    res
      .status(StatusCodes.INTERNAL_SERVER_ERROR)
      .json({ msg: "An error occurred while deleting images" });
  }
};
