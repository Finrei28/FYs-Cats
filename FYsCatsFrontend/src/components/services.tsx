...
export const getImages = async () => {
  try {
    const {
      data: { images },
    } = await axios.get(`${URL}/api/v1/images`);
...
export const addImage = async ({ name, imageValue }: ImageProps) => {
  try {
    ...
    } = await axios.post(`${URL}/api/v1/images/uploadImage`, formData, {
      headers: {
...
export const deleteImages = async (imageIds: string[], imageURLs: string[]) => {
  try {
    const response = await axios.delete(`${URL}/api/v1/images/delete`, {
...
export const sendCommentAPI = async (comment: Comment) => {
  try {
    await axios.post(`${URL}/api/v1/comments`, {
...
export const verifyEmailAPI = async (userName: string, code: string) => {
  try {
    await axios.post(`${URL}/api/v1/user/registerVerification`, {
      userName,
