...
export default function createAccount() {
  ...
  const defaultImage =
    "https://res.cloudinary.com/dpwtcr4cz/image/upload/v1724743874/FYs-Cats/tmp-1-1724743876809_qghgoq.jpg";
  const { alert, showAlert, hideAlert, success, setSuccess, setError } =
    LocalStates();
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (verifyEmail) {
      ...
      if (verification.success === "true") {
        setSuccess(true);
        setMessage(
          "You have successfully verified your email, redirecting to sign in page",
        );
        setFormData({
          userName: "",
          password: "",
    ...
    if (userName.length < 8) {
      showAlert({
        text: "Username needs to be at least 8 characters long",
        type: "danger",
      });
      return;
    }
        ...
        <form className="login-form" onSubmit={handleSubmit}>
          ...
          {formData.password && (
            <span className="criteria-container">
              <span
                className={
                  !checkPassword(formData.password).hasLetters
                    ? "criteria"
                    : "criteria met"
                }
              >
              ...
              <span
                className={
                  !checkPassword(formData.password).hasNumbers
                    ? "criteria"
                    : "criteria met"
                }
              >
              ...
              <span
                className={
                  !checkPassword(formData.password).hasSpecialCharacters
                    ? "criteria"
                    : "criteria met"
                }
              >
              ...
              <span
                className={
                  !checkPassword(formData.password).length
                    ? "criteria"
                    : "criteria met"
                }
              >
          ...
          <span onClick={() => navigate("/login")} className="forget-link">
            Log In
          </span>
        </form>
      ) : (
